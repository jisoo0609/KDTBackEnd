package org.example;

import java.util.ArrayList;
import java.util.List;
import java.util.Scanner;

public class MemoMain {
    public static void main(String[] args) {
        // 1. 사용자는 반복해서 명령을 입력한다
        // 1-1. 사용자가 1을 입력하면, 메모를 작성하는 메뉴를 선택한것
        // 1-2. 사용자가 2를 입력하면, 작성된 메모를 모두 확인하는 메뉴를 선택한 것
        // 1-3. 사용자가 q를 입력하면, 프로그램 종료 (main 메서드의 끝에 도달하는 것)

        // 입력을 받을 준비를 한다. Scanner 사용(편의를 위함)
        Scanner sc = new Scanner(System.in);

        // 메모는 문자열로 저장할 것인데, 사용자가 만든 메모의 갯수가 일정하지 않을 것을 가정한다
        List<String> memos = new ArrayList<>();

        // 리스트에 데이터 넣기 - .add()
        // 리스트에서 데이터 회수 - .get()
        // 리스트에서 데이터 제거 - .remove()

        /*
        // 3번 사용자의 입력을 받아서 memos에 저장한 뒤 하나씩 출력한다
        String userInput = sc.nextLine();
        memos.add(userInput);
        userInput = sc.nextLine();
        memos.add(userInput);
        userInput = sc.nextLine();
        memos.add(userInput);

        for (int i = 0; i < memos.size(); i++) {
            System.out.println(memos.get(i));
        }
        */

        // 사용자가 q를 입력할 때 까지 반복한다
        while (true) {
            // 사용자에게 선택지를 제시한다
            System.out.println("1. 메모 추가");
            System.out.println("2. 메모 보기");
            System.out.println("q. 종료");
            // 명령을 입력받는다
            String command = sc.nextLine();
            // 시용자가 q를 입력했는지 확인
            if ("q".equals(command)) break;
            // 1-1. 사용자가 1을 입력하면 메모를 작성하는 메뉴를 선택한 것
            else if (command.equals("1")) {
                // 사용자의 입력을 다시 받고,
                String newMemo = sc.nextLine();
                // memos에 저장한다
                memos.add(newMemo);
            }
            // 1-2. 사용자가 2를 입력하면 작성된 메모를 모두 확인하는 메뉴를 선택한 것
            else if (command.equals("2")) {
                // memos가 가진 문자열을 다 순회하며 확인하고 출력한다
                for(String memo : memos) {
                    System.out.println(memo);
                }
            }
            System.out.println();
        }
    }
}
